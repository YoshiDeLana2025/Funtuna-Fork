...
//----------------------------------------//
extern u8 YOSHCONF_ELF[];
extern int size_YOSHCONF_ELF;
//----------------------------------------//
extern u8 yoshconf_icn[];
extern int size_yoshconf_icn;
//----------------------------------------//
...
enum OPENTUNA_VARIANTS {
  ...
  FAT170,       // 0x170
  PROTOKERNELS, // this corresponds to rom 0x100 and 0x101, parrado won´t make
                // the icons, but i will leavi it here in case someone makes it
                // faster than instatuna
  UNSUPPORTED,
...
static void display_bmp(u16 W, u16 H, u32 *data) {
  gs_print_bitmap((gs_get_max_x() - W) / 2, // x
                  (gs_get_max_y() - H) / 2, // y
                  W,                        // w
                  H,                        // h
                  data                      // array
...
static int write_embed(void *embed_file, const int embed_size, char *folder,
                       char *filename, int mcport) {
  int fd, ret;
...
static int write_embed_replace(void *embed_file, const int embed_size,
                               char *folder, char *filename, int mcport) {
  int fd, ret;
  char target[MAX_PATH];
  sprintf(target, "mc%u:/%s/%s", mcport, folder, filename);
  if ((fd = open(target, O_CREAT | O_WRONLY | O_TRUNC)) < 0) {
...
static int install(int mcport, int icon_variant) {
  ...
  int ret, fd, retorno, must_kill_opl_folder = 0;
  ...
  sprintf(opl_daily_bulshit_cnf, "mc%u:/OPL/conf_elm.cfg",
          mcport); // if OPL DB settings file found delete OPL directory to
                   // avoid problems when Official OPL reads the cfg files
  if (file_exists(opl_daily_bulshit_cnf)) {
    must_kill_opl_folder = 1;
  }
  sprintf(opl_daily_bulshit_cnf, "mc%u:/OPL/conf_elmz.cfg",
          mcport); // same as above
  if (file_exists(opl_daily_bulshit_cnf)) {
    must_kill_opl_folder = 1;
  }

  sprintf(opl_daily_bulshit_cnf, "mc%u:/OPL/conf_game.cfg",
          mcport); // This isn't related to OPL DB, but the absensce of this
                   // file could indicate that OPL folder containts old settings
                   // (pre 1.1.0), delete to avoid famous black screen shit
  if (!file_exists(opl_daily_bulshit_cnf)) {
    must_kill_opl_folder = 1;
  }
  ...
  } // Installation actually requires less than this (something like 1.6MB), but
    // i left a larger size for space check since OPL will create settings and
    // icon files on first launch... (and users will innevitally load more
    // files)
  // If the files exists, we have an error:
  if (mcport == 0) {
    if (file_exists("mc0:/BXEXEC-OPENTUNA/icon.icn")) {
      return FUNTUNA_FORK_FOUND;
    }
    if (file_exists("mc0:/BXEXEC-OPENTUNA/icon.sys")) {
      return FUNTUNA_FORK_FOUND;
    }
    if (file_exists("mc0:/BXEXEC-FUNTUNA/icon.icn")) {
      return FUNTUNA_FORK_FOUND;
    }
    if (file_exists("mc0:/BXEXEC-FUNTUNA/icon.sys")) {
      return FUNTUNA_FORK_FOUND;
    }
    if (file_exists("mc0:/OPENTUNA/icon.sys")) {
      return OPENTUNA_FOUND;
    }
    if (file_exists("mc0:/FORTUNA/icon.sys")) {
      return FORTUNA_FOUND;
    }
  } else {
    if (file_exists("mc1:/BXEXEC-OPENTUNA/icon.icn")) {
      return FUNTUNA_FORK_FOUND;
    }
    if (file_exists("mc1:/BXEXEC-OPENTUNA/icon.sys")) {
      return FUNTUNA_FORK_FOUND;
    }
    if (file_exists("mc1:/BXEXEC-FUNTUNA/icon.icn")) {
      return FUNTUNA_FORK_FOUND;
    }
    if (file_exists("mc1:/BXEXEC-FUNTUNA/icon.sys")) {
      return FUNTUNA_FORK_FOUND;
    }
    if (file_exists("mc1:/OPENTUNA/icon.sys")) {
      return OPENTUNA_FOUND;
    }
    if (file_exists("mc1:/FORTUNA/icon.sys")) {
      return FORTUNA_FOUND;
    }
  }
  ...
  if (icon_variant != UNSUPPORTED) {
    if (icon_variant == SLIMS) {
      retorno = write_embed_replace(&opentuna_slims, size_opentuna_slims,
                                    "BXEXEC-OPENTUNA", "icon.icn", mcport);
      scr_printf(
          "\t\tWriting Opentuna for SLIMS and SCPH-5XXXX with ROM 1.90\n");
    } else if (icon_variant == FATS) {
      retorno = write_embed_replace(&opentuna_fats, size_opentuna_fats,
                                    "BXEXEC-OPENTUNA", "icon.icn", mcport);
      scr_printf(
          "\t\tWriting Opentuna for FATS (from SCPH-18000 to SCPH-39XXX)\n");
    } else if (icon_variant == FAT170) {
      retorno = write_embed_replace(&opentuna_fat170, size_opentuna_fat170,
                                    "BXEXEC-OPENTUNA", "icon.icn", mcport);
    ...
    retorno = write_embed_replace(&exploit_sys, size_exploit_sys,
                                  "BXEXEC-OPENTUNA", "icon.sys", mcport);
    if (retorno < 0) {
      return 6;
    }

    if (!((fd = open(version_manifest_path, O_CREAT | O_WRONLY | O_TRUNC)) <
          0)) {
    ...
    static sceMcStDateTime
        maximahora; // Maxium Timestamp, for the ones who does not speak Spanish
  ...
  retorno = write_embed_replace(&YOSHBOOT_ELF, size_YOSHBOOT_ELF, "BOOT",
                                "BOOT.ELF", mcport);
  if (retorno < 0) {
    return 6;
  }
  retorno = write_embed_replace(&yoshboot_icn, size_yoshboot_icn, "BOOT",
                                "yoshboot.icn", mcport);
  if (retorno < 0) {
    return 6;
  }
  retorno = write_embed_replace(&yoshboot_sys, size_yoshboot_sys, "BOOT",
                                "icon.sys", mcport);
  if (retorno < 0) {
    return 6;
  }
  ...
  retorno = write_embed_replace(&ule_elf, size_ule_elf, "SYS-CONF", "ULE.ELF",
                                mcport);
  if (retorno < 0) {
    return 6;
  }
  ...
  retorno = write_embed_replace(&pops_icn, size_pops_icn, "POPSTARTER",
                                "pops.icn", mcport);
  if (retorno < 0) {
    return 6;
  }
...
int main(int argc, char *argv[]) {
  ...
  if ((fdn = open("rom0:ROMVER", O_RDONLY)) > 0) // Reading ROMVER
  {
    ...
    romver[4] = '\0';
    ROM_VERSION =
        strtoul(romver, NULL, 16); // convert ROM version to unsigned long int
                                   // for further use on automatic Install
  }
  ...
  int iz = 1, mcport;
  int menuactual = 101; // 101: Initial Menu, 102: Installing (not needed), 103:
                        // Error, 104: Done
  // printf("FunTuna Fork Installer\n\nPress L1 to install on memory card 1 |
  // Press R1 to install on memory card 2");
  ...
  while (1) {
    ...
    else if ((new_pad & PAD_START) && (menuactual == 103)) {
      PS2_browser();
